...
export default function AdminPoints() {
  ...
  const adjustMutation = trpc.points.adminAdjust.useMutation({
    onSuccess: () => { toast.success("积分操作成功"); utils.user.adminList.invalidate(); setOpen(false); setAmount(""); setNote(""); },
    ...
    <AdminLayout>
      <div className="space-y-6">
        <div>
          ...
          <p className="text-muted-foreground text-sm mt-1">管理用户积分余额，手动增减积分</p>
        </div>
          ...
          <DialogContent className="bg-card border-border">
            <DialogHeader>
              <DialogTitle className="flex items-center gap-2">
                ...
                {isAdd ? "增加" : "扣减"}积分 - 用户 #{selectedUserId}
              </DialogTitle>
            </DialogHeader>
            <div className="space-y-4 mt-2">
              <div className="space-y-2">
                <Label>积分数量</Label>
                <Input type="number" min="1" placeholder="请输入积分数量" value={amount} onChange={(e) => setAmount(e.target.value)} className="bg-input border-border" />
              </div>
              <div className="space-y-2">
                <Label>操作原因</Label>
